...
# 
##############################################
make_usgs_gage_list=function(siteNo = "02486000",
...
USGS02486000=make_usgs_gage_list(siteNo = "02486000")
USGS02488500=make_usgs_gage_list(siteNo ="02488500" )
USGS02489000=make_usgs_gage_list(siteNo ="02489000" )


ab_ll=rbind(USGS02486000$site,
            USGS02488500$site,
            USGS02489000$site)
class(ab_ll)
ab_ll@proj4string
proj4_utm = paste0("+proj=utm +zone=",
                   trunc((180+coordinates(USGS02489000$site)[1])/6+1), 
...
# USGS H
url="https://prd-tnm.s3.amazonaws.com/StagedProducts/Hydrography/NHD/HU8/HighResolution/Shape/NHD_H_03180002_HU8_Shape.zip"
curl_download(url,"NHD_H_03180002_HU8_Shape.zip")
unzip("NHD_H_03180002_HU8_Shape.zip",exdir="03180002")
streams=readOGR("03180002/Shape/NHDFlowline.dbf")
...
#
A=SpatialPoints(USGS02486000$site)# Up gradient site TINKER CREEK
B=SpatialPoints(USGS02489000$site) # Down gradient site ROA River atNiagara
...
cropmydem=crop(mydem,extend(extent(ab_utm),1000))
...
USGS02486000$site$L=SpatialLinesLengths(AtoB) # km to m
USGS02486000$site$L # reach length in m
#
#
...
#
USGS02486000$site$slope=(extract(mydem,A_utm)-
                           extract(mydem,B_utm))/USGS02486000$site$L
USGS02486000$site$slope

#Width B
USGS02486000$flowdata$B=(USGS02486000$site$man_n*
                           USGS02486000$flowdata$cms)/(USGS02486000$flowdata$depth_m^(5/3)*
                                                         sqrt(USGS02486000$site$slope))
head(USGS02486000$flowdata)
#  agency_cd	site_no        	dateTime X_00060_00000 X_00060_00000_cd
...
plot(USGS02486000$flowdata$dateTime,USGS02486000$flowdata$B, main="TINKER CREEK TO ROANOKE RIVER AT NIAGARA, VA")
...
#
plot(USGS02486000$flowdata$cms,USGS02486000$flowdata$depth_m, main="TINKER CREEK TO ROANOKE RIVER AT NIAGARA, VA")

# ck
#USGS0205551460$flowdata$ck = ???
# ANS
USGS02486000$flowdata$ck =
  5/3*sqrt(USGS02486000$site$slope)/USGS02486000$site$man_n*
  (USGS02486000$flowdata$depth_m^(2/3))
#
#USGS0205551460$flowdata$dt = ???
USGS02486000$flowdata$dt =
  USGS02486000$site$L/USGS02486000$flowdata$ck

plot(USGS02486000$flowdata$dateTime,USGS02486000$flowdata$dt)
USGS02486000$flowdata$outTime=USGS02486000$flowdata$dateTime+
  USGS02486000$flowdata$dt

# Find beginning of  Waves
USGS02486000$flowdata$newwave=
  USGS02486000$flowdata$cms *1.1 <
  data.table::shift(USGS02486000$flowdata$cms)
summary(USGS02486000$flowdata$newwave)
# Add plot of the point found
len=length(USGS02486000$flowdata$newwave)
USGS02486000$flowdata$newwave[is.na(USGS02486000$flowdata$newwave)]=F
# Removes repeated finds by going through loop backwords
#IF today is true and tomorrow is true then remove today's true
for (i in seq(len,2)){
  print(i)
  if(USGS02486000$flowdata$newwave[i]==T &
     USGS02486000$flowdata$newwave[i-1]==T){
    USGS02486000$flowdata$newwave[i]=F
  }
}
plot(USGS02486000$flowdata$dateTime,USGS02486000$flowdata$cms,type="l")
points(USGS02486000$flowdata$dateTime[USGS02486000$flowdata$newwave],
       USGS02486000$flowdata$cms[USGS02486000$flowdata$newwave],col=2)

# Find the time locations where waves begin
which(USGS02486000$flowdata$newwave == TRUE)
plot(USGS02486000$flowdata$dateTime,USGS02486000$flowdata$cms,
     type="l",xlim=c(USGS02486000$flowdata$dateTime[15605],
                     USGS02486000$flowdata$dateTime[15605+100]),
              ylim=c(0,20),
     main="PEARL RIVER AT JACKSON-USGS02486000",xlab="Time",ylab="Q_cms",xaxt="n")

lines(USGS02486000$flowdata$outTime,USGS02486000$flowdata$cms,col=2)
axis(1, USGS02486000$flowdata$dateTime,
     format(USGS02486000$flowdata$dateTime, "%b-%d"))
legend("topright",                                 
...
#Homework plot
ggplot() +
  geom_line(aes(x=USGS02486000$flowdata$dateTime, y = USGS02486000$flowdata$cms, 
                colour="black")) +
  geom_line(aes(x=USGS02486000$flowdata$outTime, y = USGS02486000$flowdata$cms,
                colour="red")) +
  labs(x = 'Time', y = 'Flow (mm)')+ 
  xlim(USGS02486000$flowdata$dateTime[16900],USGS02486000$flowdata$dateTime[16900+50])+
  ...
  scale_color_identity(name = "Legend",
                       ...
                       guide = "legend")
#Answers
USGS02486000[["flowdata"]][["dt"]][15605]
USGS02486000[["flowdata"]][["ck"]][15605]
